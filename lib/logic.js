...
function TradeAirPollutionDataToTokens(AirPollutionDataReceived) {
    if(AirPollutionDataReceived.fromWallet.balance < AirPollutionDataReceived.amount) {
        throw new Error('Insufficient funds!');
    }
    //update values of the wallets
  	AirPollutionDataReceived.fromWallet.balance -= AirPollutionDataReceived.amount;
    AirPollutionDataReceived.toWallet.balance += AirPollutionDataReceived.amount;
      
      var airpollutiondata = AirPollutionDataReceived.airpollutiondata;
      AirPollutionDataReceived.airpollutiondata.owner = AirPollutionDataReceived.newOwner;
    ...
    return getAssetRegistry('org.acme.biznet.AirPollutionData')
        .then(function(assetRegistry) {
            return assetRegistry.update(AirPollutionDataReceived.airpollutiondata);
        })
        .then(function(){
            return getAssetRegistry('org.acme.biznet.Wallet')
                .then(function (assetRegistry) {
                    return assetRegistry.updateAll(AirPollutionDataReceived.fromWallet,AirPollutionDataReceived.toWallet);
                });
        });
        var tradeAirPollutionDataNotification = getFactory().newEvent('org.acme.biznet', 'TradeAirPollutionDataNotification');
        tradeAirPollutionDataNotification.airpollutiondata = AirPollutionDataReceived.airpollutiondata;
        emit(tradeAirPollutionDataNotification);
        
        console.log('Luftverschmutzungsdaten mit ID' + airPollutionDataId + ' wurde an ' + BuyerId + ' verkauft.');
}
